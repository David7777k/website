...
async def list_users(page: int = 1, limit: int = 25, search: Optional[str] = None, role: Optional[str] = None, current_admin: User = Depends(admin_required)):
    query = {}
    if role:
        query['role'] = role
    if search:
    ...
    # Normalize items into UserSummary-like dicts
    results = []
    for it in items:
        results.append({
